...
def create_monitor(payload: MonitorCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> MonitorRead:
...
def get_monitor_stats(
    ...
    monitor = db.query(Monitor).filter(Monitor.id == monitor_id, Monitor.user_id == current_user.id).one_or_none()
    if not monitor:
...
def update_monitor(
    monitor_id: int, payload: MonitorUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
) -> MonitorRead:
    m = (
        db.query(Monitor)
        .options(joinedload(Monitor.tags))
...
def get_monitor(monitor_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> MonitorRead:
    m = (
        db.query(Monitor)
        .options(joinedload(Monitor.tags))
...
def delete_monitor(monitor_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
...
def get_sla(monitor_id: int, window: str = Query(default="24h"), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> dict:
    m = db.query(Monitor).filter(Monitor.id == monitor_id, Monitor.user_id == current_user.id).one_or_none()
    if not m:
        raise HTTPException(status_code=404, detail="monitor not found")
